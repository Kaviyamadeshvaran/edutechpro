...
const AddCourse = () => {
   ...
   const handleSubmit = async (e) => {
      ...
      addCourse.sections.forEach((section) => {
         ...
         formData.append("S_link[]", section.S_link);
         if (section.S_content instanceof File) {
            formData.append("S_content", section.S_content); // Note: Files will naturally be treated as arrays if multiple
         }
      });
               ...
               <div key={index} className="d-flex flex-column mb-4 border rounded-3 border-3 p-3 position-relative">
                  ...
                  <Row className='mb-3'>
                     ...
                     <Form.Group as={Col} controlId="formGridLink">
                        <Form.Label>Section Link</Form.Label>
                        <Form.Control
                           name={`S_link`}
                           value={section.S_link}
                           onChange={(e) => handleChangeSection(index, e)}
                           type="text"
